...
- Failed database writes being reported as successes. `WriteArchimedes` ran its statement with `Query` and closed the rows without reading them, and pgx surfaces a statement's execution error — a constraint violation, a trigger raising — only through those rows. `StartPump`, `StopPump` and `UpdateVolume` therefore returned `nil` for writes the database had rejected, so the processors logged nothing and the event was lost with no trace anywhere. Writes now go through `Exec`, which reports what the database refused. A write that is accepted and matches no row still succeeds silently, which is what lets a stop for a pump with no open run record nothing.
- A read API that could not open its port being logged as running. `Serve` called `ListenAndServe` in a goroutine and discarded its error, so a port already in use left the process up and consuming MQTT with no HTTP API and nothing said about it. The port is now opened before `Serve` returns, and the failure is logged and returned to `main`, which stops.
